...
export interface RemoteRenderJob {
  jobId: string; candidateId: string; source: string; sourceHash: string; deadline: string;
}
...
export class RemoteRenderJobsRepository {
  ...
  async enqueue(id: string, request: CandidateRenderRequest): Promise<void> {
    const result = await this.pool.query(
      `INSERT INTO browser_render_jobs (id, owner_id, project_id, candidate_id, source_hash, delivery_mode, claim_deadline, deadline)
       SELECT $1, $2, c.project_id, c.id, c.source_hash, 'remote-mcp', now() + interval '15 seconds', now() + interval '90 seconds'
  ...
  async claim(projectId: string, sessionId: string, tokenHash: string): Promise<RemoteRenderJob | undefined> {
    await this.recover();
    const result = await this.pool.query<{ id: string; candidate_id: string; source: string; source_hash: string; deadline: Date }>(
      `WITH next_job AS (
         ...
         WHERE j.owner_id = $1 AND p.owner_id = $1 AND j.project_id = $2 AND j.delivery_mode = 'remote-mcp'
       ...
       ) SELECT claimed.id, claimed.candidate_id, c.source, claimed.source_hash, claimed.deadline
    ...
    return job ? { jobId: job.id, candidateId: job.candidate_id, source: job.source, sourceHash: job.source_hash, deadline: new Date(job.deadline).toISOString() } : undefined;
  }

  async outcome(id: string): Promise<PersistedRenderOutcome | undefined> {
    await this.expire();
    const result = await this.pool.query<PersistedRenderOutcome>("SELECT state, completion FROM browser_render_jobs WHERE id = $1 AND owner_id = $2 AND delivery_mode = 'remote-mcp'", [id, this.ownerId]);
    return result.rows[0];
  }

  async cancel(id: string): Promise<void> {
    await this.pool.query("UPDATE browser_render_jobs SET state = 'CANCELLED', updated_at = now() WHERE id = $1 AND owner_id = $2 AND delivery_mode = 'remote-mcp' AND state = 'PENDING'", [id, this.ownerId]);
  }

  private async expire(): Promise<void> {
    await this.pool.query(`UPDATE browser_render_jobs SET state = 'EXPIRED', updated_at = now()
      WHERE owner_id = $1 AND delivery_mode = 'remote-mcp' AND state = 'PENDING'
  ...
  async recover(): Promise<void> {
    ...
    await this.pool.query("DELETE FROM browser_render_jobs WHERE owner_id = $1 AND delivery_mode = 'remote-mcp' AND state <> 'PENDING' AND updated_at < now() - interval '1 day'", [this.ownerId]);
  }
}
