...
export async function provisionCodexClient(pool: Pool, callback: string): Promise<string> {
  ...
  const result = await pool.query<{ client_id: string }>(
    `INSERT INTO oauth_client (id, client_id, name, redirect_uris, token_endpoint_auth_method,
     ...
     ON CONFLICT (client_id) DO UPDATE SET
       redirect_uris = ARRAY(SELECT DISTINCT unnest(oauth_client.redirect_uris || EXCLUDED.redirect_uris)), updated_at = now()
     RETURNING client_id`,
